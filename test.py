...
def test(local_rank, world_size, args):
    ...
    val_gthsi = HSIdataset(root=Dataset_path, split='test2')
    val_data_set = SyntheticHSI(val_gthsi, downfactor=sr_ratio, crop_sz=args.test_size, center_crop=True, gray_max=gray_max, offset_max=args.offset_max)
    ...
    with torch.no_grad():
        for k, data in enumerate(val_loader):
            samplesLR, samplesLR_blur, _, gt,numbers = data
            ...
            numbers = numbers[0]
            if local_rank == 0:
                safe_mkdir(os.path.join(result_dir_SR, "{:01d}".format(numbers)))
                safe_mkdir(os.path.join(result_dir_LR, "{:01d}".format(numbers)))
                safe_mkdir(os.path.join(result_dir_GT, "{:01d}".format(numbers)))

            if local_rank == 0:
                HRHSI = HRHSI[0, ...]
                gt = gt[0, ...]
                samplesLR_blur = samplesLR_blur[0, ...]
                b, num_spec, h, w = samplesLR.shape
                for n in range(num_spec):
                    HR = HRHSI[n, ...]
                    HR = HR.detach().cpu().numpy().squeeze()
                    HR = (HR * 65535).astype('uint16')
                    imwrite(os.path.join(result_dir_SR, "{:01d}".format(numbers), "HR_{:02d}.png".format(n)), HR)
                    LR = samplesLR_blur[n, ...]
                    LR = LR.detach().cpu().numpy().squeeze()
                    LR = (LR * 65535).astype('uint16')
                    imwrite(os.path.join(result_dir_LR, "{:01d}".format(numbers), "LR_{:02d}.png".format(n)), LR)
                    '''
                    GT = gt[n, ...]
                    GT = GT.detach().cpu().numpy().squeeze()
                    GT = (GT * 65535).astype('uint16')
                    imwrite(os.path.join(result_dir_GT, "{:01d}".format(numbers), "GT_{:02d}.png".format(n)), GT)'''
...
if __name__ == '__main__':
    ...
    parser.add_argument("-sgt", "--save_GT", help="save GT image", type=str, default='/GT_file')
